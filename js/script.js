...
const CONFIG = {
  ...
  // World map pins — top/left are percentages (0-100) of the map image.
  mapPins: [
    { label: "Where we met", top: 42, left: 48, type: "visited" },
    { label: "That unforgettable trip", top: 30, left: 22, type: "visited" },
    { label: "Weekend getaway", top: 60, left: 68, type: "visited" },
    { label: "Dream: Santorini", top: 34, left: 53, type: "dream" },
    { label: "Dream: Kyoto", top: 38, left: 84, type: "dream" }
  ],
...
function initCursor() {
  ...
  const hoverables = "a, button, .masonry-item, .flip-card, .map-pin, input, textarea, .polaroid, .swatch";
...
function initWorldMap() {
  const container = $("#map-pins");
  if (!container) return;
  container.innerHTML = ""; // idempotent — safe to call again after a draft restore
  CONFIG.mapPins.forEach((pin) => {
    const el = document.createElement("div");
    el.className = `map-pin ${pin.type === "dream" ? "dream" : "visited"}`;
...
function cleanNodeForExport(node) {
  ...
  node.querySelectorAll(".img-edit-badge").forEach((el) => el.remove());
  node.querySelectorAll(".confetti-piece, .firework-particle").forEach((el) => el.remove());
  const pins = node.querySelector("#map-pins");
